...
def load_tpch(dbmodule, tpchdir):
	...
	dbmodule.execute_file('${DBNAME}.load.sql.tmp'.replace('${DBNAME}', dbname))
	...
	dbmodule.execute_file('${DBNAME}.analyze.sql'.replace('${DBNAME}', dbname))
	dbmodule.execute_file('${DBNAME}.constraints.sql'.replace('${DBNAME}', dbname))
	os.chdir(CURRENTDIR)
